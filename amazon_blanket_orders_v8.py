...
<style>
    ...
    .nav-link { display:block; padding:12px 15px; margin:4px 0; border-radius:10px; color:#a0aec0; text-decoration:none; transition:all .2s; cursor:pointer; }
    .nav-link:hover { background:#2d3748; color:#e4e6eb; text-decoration:none; }
    .nav-link.active { background:linear-gradient(135deg,#667eea 0%,#764ba2 100%); color:#fff; }

    [data-testid="stMetric"] { background:linear-gradient(135deg,#1e2432 0%,#252d3d 100%); border:1px solid #2d3748; padding:25px 20px; border-radius:16px; border-left:3px solid #667eea; }
    [data-testid="stMetric"]:hover { transform:translateY(-3px); box-shadow:0 10px 30px rgba(102,126,234,.2); transition:all .3s; border-color:#667eea; }
    [data-testid="stMetric"] label { font-size:.85em!important; color:#a0aec0!important; font-weight:600!important; text-transform:uppercase; letter-spacing:.5px; }
    [data-testid="stMetric"] [data-testid="stMetricValue"] { font-size:2.5em!important; font-weight:700!important; color:#e4e6eb!important; }

    h1 { color:#e4e6eb; font-weight:700; padding-bottom:15px; border-bottom:3px solid #667eea; margin-bottom:30px; }
    h2 { color:#e4e6eb; font-weight:600; margin:40px 0 20px; }
    h3 { color:#cbd5e0; font-weight:600; margin-bottom:15px; }

    .stButton button { background:linear-gradient(135deg,#667eea 0%,#764ba2 100%); color:#fff; border:none; border-radius:10px; padding:12px 24px; font-weight:600; transition:all .3s; width:100%; }
    .stButton button:hover { transform:translateY(-2px); box-shadow:0 8px 20px rgba(102,126,234,.4); }

    [data-testid="stFileUploader"] { background:#1a1f2e; padding:40px; border-radius:12px; border:2px dashed #2d3748; }
    [data-testid="stFileUploader"]:hover { border-color:#667eea; background:#1e2432; }
    [data-testid="stFileUploader"] label { color:#e4e6eb!important; }
    [data-testid="stFileUploader"] section { border-color:#2d3748!important; }

    .stAlert { background:linear-gradient(135deg,#667eea20,#764ba220)!important; border:1px solid #667eea40!important; border-radius:10px; border-left:4px solid #667eea!important; color:#cbd5e0!important; }

    [data-baseweb="notification"] { background:#1a1f2e!important; border:1px solid #48bb78!important; color:#e4e6eb!important; }

    [data-testid="stDataFrame"] { border-radius:12px; overflow:hidden; }
    [data-testid="stDataFrame"] table { background:#1a1f2e!important; color:#e4e6eb!important; }
    [data-testid="stDataFrame"] thead tr th { background:#2d3748!important; color:#e4e6eb!important; }
    [data-testid="stDataFrame"] tbody tr { background:#1e2432!important; color:#cbd5e0!important; }
    [data-testid="stDataFrame"] tbody tr:hover { background:#252d3d!important; }

    [data-testid="stExpander"] { background:#1a1f2e!important; border-radius:10px; border:1px solid #2d3748!important; margin-bottom:10px; }
    [data-testid="stExpander"] [data-testid="stMarkdownContainer"] { color:#e4e6eb!important; }

    .stProgress > div > div { background:linear-gradient(90deg,#667eea 0%,#764ba2 100%); }

    .stDownloadButton button { background:linear-gradient(135deg,#48bb78 0%,#38a169 100%); color:#fff; border:none; border-radius:10px; padding:10px 20px; font-weight:600; width:100%; }
    .stDownloadButton button:hover { transform:translateY(-1px); box-shadow:0 5px 15px rgba(72,187,120,.4); }
    ...
    input, textarea, select { background:#1a1f2e!important; color:#e4e6eb!important; border:1px solid #2d3748!important; }
    .stMarkdown { color:#cbd5e0!important; }

    .status-indicator { display:inline-flex; align-items:center; gap:8px; background:#2d3748; padding:6px 12px; border-radius:20px; font-size:.85em; }
...
def normalize_order_id(s: str | None) -> str | None:
    ...
    if not s:
        return None
    m = ORDER_ID_FLEX_RE.search(str(s))
    if m:
...
def get_bobbin_color(thread_color):
    """Determine bobbin color based on thread color (null-safe)."""
    if not thread_color:
        return 'White Bobbin'
...
def draw_checkbox(canvas_obj, x, y, size, is_checked):
    ...
    canvas_obj.rect(x, y, size, size, stroke=1, fill=1 if is_checked else 0)
...
# --------------------------------------
def build_mfg_index_map(df) -> dict[str, list[int]]:
    """
    Returns {order_id: [mfg_page_indices...]} assigning contiguous page ranges per order
    in first-seen order of df. Uses sum of Quantity_Int per order.
    """
    local = df.copy()
    if 'Quantity_Int' not in local.columns:
        local['Quantity_Int'] = local['Quantity'].astype(int)

    order_first_seen, seen = [], set()
    for oid in local['Order ID']:
        if oid not in seen:
            seen.add(oid)
            order_first_seen.append(oid)

    pages_needed = {oid: int(local.loc[local['Order ID'] == oid, 'Quantity_Int'].sum())
    ...
    for oid in order_first_seen:
        n = pages_needed[oid]
        m[oid] = list(range(cursor, cursor + n))
        cursor += n
    return m

# --------------------------------------
# FIXED: Label Merging Function (manifest-driven, shift-proof)
# --------------------------------------
...
def generate_manufacturing_labels(dataframe):
    """Generate one page PER UNIT (Quantity)."""
    buf = BytesIO()
    ...
    for _, row in local.iterrows():
        qty = int(row['Quantity_Int'])
        for unit_idx in range(1, qty + 1):
            y = top
            c.setFont("Helvetica-Bold", 14)
            c.drawString(left, y, f"Order ID: {row['Order ID']}")
            c.drawRightString(right, y, f"Qty: {qty}  (Unit {unit_idx}/{qty})")
            y -= 0.25 * inch
            ...
            text_y -= 0.25 * inch
            c.setFont("Helvetica-BoldOblique" if is_beanie_checked else "Helvetica-Bold", 14)
            ...
            text_y -= 0.25 * inch
            c.setFont("Helvetica-BoldOblique" if is_gift_box_checked else "Helvetica-Bold", 14)
            ...
            text_y -= 0.25 * inch
            c.setFont("Helvetica-BoldOblique" if is_gift_note_checked else "Helvetica-Bold", 14)
...
if uploaded:
    ...
    for page_text in all_pages:
        ...
        # IDs & date
        m_id = re.search(r"Order ID:\s*([^\n]+)", page_text)
        order_id = normalize_order_id(m_id.group(1).strip()) if m_id else None
        m_date = re.search(r"Order Date:\s*([A-Za-z]{3,},?\s*[A-Za-z]+\s*\d{1,2},?\s*\d{4})", page_text)
        order_date = m_date.group(1).strip() if m_date else ""
    ...
    **Shift-Proof Merge Instructions**
    ...
    if shipping_labels_upload and st.session_state.manufacturing_labels_buffer:
        ...
        with col_merge1:
            if st.button("🔀 Merge Labels Now", type="primary", use_container_width=True):
                with st.spinner("Merging shipping and manufacturing labels..."):
                    ...
                    if merged_pdf:
                        st.success(f"✅ Merged {num_shipping} shipping pages with {num_manufacturing} manufacturing pages")
                        st.download_button(
        ...
        with col_merge2:
            st.metric("Distinct Orders (DF)", df['Order ID'].nunique())
